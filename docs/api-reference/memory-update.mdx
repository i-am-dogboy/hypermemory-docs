...
The open-source server's `memory_update` tool also supports `merge_data` (bool, default `True` for deep-merge vs replace), `add_relationships`, `remove_relationships`, `assets`, `source_uri`, and `force_extra_data`.
